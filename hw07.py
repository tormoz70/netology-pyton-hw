...
# Задание по Psycopg2
# --------------------------------------------------------------
logger.info("Задание по Psycopg2")
logger.info("--------------------------------------------------------------")
...
cursor.execute("DROP TABLE IF EXISTS movies_top")
conn.commit()
...
logger.info("Задание по SQLAlchemy")
logger.info("--------------------------------------------------------------")
...
top_rated_query = session.query(MoviesTop).filter(MoviesTop.ratings_avg>3.5).order_by(MoviesTop.ratings_avg.asc()).limit(10)
...
logger.info("Задание по PyMongo")
logger.info("--------------------------------------------------------------")
...
tags_df = tags_df.groupby(['tags'])['movieid'].count().reset_index().sort_values('count', ascending=False)
